...
class LinkedList:
    ...
    def insert(self, data, i):
        ...
        # 保存当前插入位置的节点
        current = None if previous.next is None else previous.next
    ...
    def remove(self, i):
        previous = self.get(i - 1)
        # 保存删除节点的数据来返回
        data = None if previous.next is None else previous.next.data
        # 前一个节点指向删除节点的后一个节点
        previous.next = None if previous.next is None else previous.next.next
        self.length -= 1
        return data
